...
const dispatchUpdate = function (optimistic, collectionName, channels, docIds, fields) {
    if (optimistic) {
        docIds.forEach(docId => {
            compensateForLatency(channels, collectionName, Events.UPDATE, {_id: docId}, fields);
        });
    }
...
const dispatchRemove = function (optimistic, collectionName, channels, docIds) {
    if (optimistic) {
        docIds.forEach((docId) => {
            compensateForLatency(channels, collectionName, Events.REMOVE, {_id: docId})
        });
    }
...
const dispatchInsert = function (optimistic, collectionName, channels, docId) {
    if (optimistic) {
        // OPTIMISTIC UI CODE
        compensateForLatency(channels, collectionName, Events.INSERT, {_id: docId});
    }
