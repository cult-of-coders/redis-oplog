...
export default class Mutator {
    ...
    static update(Originals, selector, modifier, _config, callback) {
        ...
        const config = getMutationConfig(this._name, _config);
        console.log('update Config1',selector, modifier, _config);
        if (!config.pushToRedis) {
        ...
        var fieldsObj = {types:1};
        fields.forEach(function (val) {
          fieldsObj[val] = 1;
        })
        const findOptions = {fields: fieldsObj, transform: null};
        if (!config.multi) {
            findOptions.limit = 1;
        }
        console.log('findOptions', findOptions)
        let docs = this.find(selector, findOptions).fetch();

        if (_config && _config.upsert) {
            return Mutator._handleUpsert.call(this, Originals, selector, modifier, config, callback, docs, fields)
        }
        ...
        console.trace('update Config2',_config,selector, modifier,updateSelector);
        try {
            ...
            console.trace('update fields',this._name, fields, config._channels,docs);
     ...
     * @param docIds
     */
    static _handleUpsert(Originals, selector, modifier, config, callback, docs, fields) {
        try {
