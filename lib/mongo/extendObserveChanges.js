...
export default function (cursor, selector, options) {
    ...
    const create = function(observer) {
        ...
        // We cache the _publicationEntry to make it work with: peerlibrary:reactive-publish package
        // Otherwise it will lead into an infinite recursion because the package overrides fetch()
        if (cursor._publicationEntry) {
            pe = cursor._publicationEntry;
        } else {
            pe = cursor._publicationEntry = new PublicationEntry(null, [cursor]);
        }
