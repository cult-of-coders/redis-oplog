...
describe('Processors - Requery Action', function () {
    ...
    it('should work without filters', function () {
        ...
        requery(oc);
        assert.equal(oc.store.size(), 11);

        Collection.remove({_id: newId});
        requery(oc);
        assert.equal(oc.store.size(), 10);
    });
    ...
    it('should work with filters', function () {
        ...
        requery(oc);
        assert.equal(oc.store.size(), 6);

        Collection.remove({_id: newId});
        requery(oc);
        assert.equal(oc.store.size(), 5);
    ...
    it('should properly work with limit sort', function () {
        ...
        requery(oc);
        assert.equal(oc.store.size(), 3);
        assert.isObject(oc.store.get(newId));

        Collection.remove({_id: newId});
        requery(oc);
        assert.equal(oc.store.size(), 3);
        assert.isUndefined(oc.store.get(newId));
    })
});
