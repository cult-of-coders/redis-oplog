 ...
 */
export default async function (observableCollection, event, doc, modifier, modifiedTopLevelFields) {
    switch (event) {
        case Events.INSERT:
            await handleInsert(observableCollection, doc);
            break;
        case Events.UPDATE:
            await handleUpdate(observableCollection, doc, modifier, modifiedTopLevelFields);
            break;
        case Events.REMOVE:
 ...
 * @param modifiedTopLevelFields
 */
const handleUpdate = async function (observableCollection, doc, modifier, modifiedTopLevelFields) {
    await observableCollection.changeSynthetic(doc._id, modifier, modifiedTopLevelFields);
};
