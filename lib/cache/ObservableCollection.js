...
export default class ObservableCollection {
    ...
    constructor(observer, cursor, config = {}) {
        ...
        this.testDocEligibility = _.keys(this.selector).length ? new Matcher(this.selector) : null;
    ...
    isEligibleByDB(doc) {
      //return true
        //console.log(this.testDocEligibility);
        console.log('isEligibleByDB', this.selector,this.observer.id,this.channels)
        if (this.testDocEligibility) {
            // Using Mingo to test query instead of using mongodb.
            var obj = Object.assign(doc, this.store[doc._id])

            const isMatch = this.testDocEligibility.documentMatches(obj).result;
            console.log('isEligibleByDB', isMatch, obj);
            return isMatch;
    ...
    change(docId, modifiedFields) {
        ...
        console.log('changedTopLevelFields',_.keys(changedTopLevelFields))
     ...
     */
    remove(docId) {
      console.log('observable store remove', docId,_.keys(this.store).length, this.store[docId])
        if (this.store[docId]) {
