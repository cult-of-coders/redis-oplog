...
export default class ObservableCollection {
    ...
    change(docId, modifiedFields) {
        ...
        try {
            const result = this._changeNormal(storedDoc, modifiedFields);
            topLevelFieldsArray = _.keys(result);
            storedDoc = this._getExtendedStoreItem(docId, result);
        } catch (e) {
            if (e === 'not-found') {
    ...
    _changeNormal(storedDoc, modifiedFields) {
        ...
        let result = this.collection.findOne(storedDoc._id, {
            fields,
            transform: null,
        });
