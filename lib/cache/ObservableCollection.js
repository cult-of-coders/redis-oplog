...
export default class ObservableCollection {
    ...
    _changeNormal(storedDoc, modifiedFields) {
        let opts = {};
        let runProjectionExclusion = false;

        if (this.options.fields) {
            if (this.isFieldsProjectionByExclusion) {
                runProjectionExclusion = true;
                opts.fields = filterDisallowedFields(this.fieldsArray, modifiedFields)
            } else {
                opts.fields = filterAllowedFields(this.fieldsArray, modifiedFields);
            }
        } else {
            opts.fields = {};
            modifiedFields.forEach(modifiedField => opts.fields[modifiedField] = 1);
        }

        let result = this.collection.findOne(storedDoc._id, opts);
        if (!result) {
            throw 'not-found';
        }

        // the snapback concept is done here because
        // our lovely mongodb allows updating list of arrays like you would update objects
        // and because deepExtend overrides arrays
        // we need the full value of the array and then we apply runProjectionExclusion
        const snapbacks = getSnapbackFields(result, _.keys(opts.fields));
        if (snapbacks.length) {
            let snapbacksFields = {};
            snapbacks.forEach(field => snapbacksFields[field] = 1);
            let snapbackResult = this.collection.findOne(storedDoc._id, {fields: snapbacksFields});
            if (!snapbackResult) {
                throw 'not-found';
            }
            deepExtend(result, snapbackResult);
            runProjectionExclusion = true;
        }
        ...
        processUndefined(result, opts.fields);

        if (this.options.fields && runProjectionExclusion) {
