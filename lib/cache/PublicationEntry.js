...
export default class PublicationEntry {
    ...
    addObserver(observer) {
        if (observer.connection) {
            let observerExists = !!_.find(this.observers, o => {
                if (o.connection) {
    ...
    send(action, ...args) {
        ...
        // Note this won't be as performant with publishComposite.
        if (invoke) {
            // we send first to invoke.connection.id
            ...
            const currentObserver = _.find(this.observers, o => {
                return o.connection && o.connection.id == currentId;
            });

            if (currentObserver) {
                currentObserver[action].call(currentObserver, ...args);
