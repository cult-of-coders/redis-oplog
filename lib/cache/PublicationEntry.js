...
export default class PublicationEntry {
    ...
    /**
     * Sends changes to a specific observer, used for optimistic-ui
     *
     * @param id
     * @param action
     * @param args
     */
    sendToObserver(id, action, ...args) {
        const observer = _.find(this.observers, o => o.session.id === id);

        observer && observer[action].call(observer, ...args);
    }
