...
describe('Unit-Test ObservableCollection', function () {
    ...
    it('should dispatch events to the observer: update', function (done) {
        let doc = {
            _id: 'XXX',
            data: 1
        };
        ...
        const observer = {
            send(event, collectionName, id, data) {
                ...
                assert.equal(collectionName, 'test_observable_collection');
                assert.equal(id, 'XXX');
                assert.equal(data.data, 2);
                assert.equal(data.otherData, 1);
        ...
        oc.observer = observer;
        oc.change(doc._id, {data: 2, otherData: 1}); // should not fail
    })
});