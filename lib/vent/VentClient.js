...
export default class VentClient {
    ...
    listen(ddpConnection) {
        ddpConnection._stream.on('message', (raw_msg) => {
            ...
            const search = `{"msg":"changed","collection":"${VentConstants.PREFIX}`;
            if (raw_msg.indexOf(search) !== 0) {
                return;
            }

            const msg = DDPCommon.parseDDP(raw_msg);
            if (msg && msg.msg === 'changed' && msg.collection.indexOf(VentConstants.PREFIX) === 0) {
                const subscription = this.store[msg.collection];
                if (subscription) {
                    subscription.handle(msg.fields[VentConstants.EVENT_VARIABLE]);
                }
            }
        });
    }
