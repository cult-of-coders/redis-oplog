/* globals Kadira */
import RedisPipe, { Strategy } from '../constants';
...
export default class RedisSubscriber {
     ...
     * @param documentMap 
     */
    process(events, documentMap) {
        this.processor.call(null, this.observableCollection, events, documentMap);
        if (events.length && Kadira) {
            events.forEach(event => {
                const op = event[RedisPipe.EVENT];
                if (op) {
                    Kadira.models.pubsub.trackDocumentChanges(this.observableCollection.__driver__._ownerInfo, { op });
                }
            })
        }
    }
